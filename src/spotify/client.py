"""
src/spotify/client.py — Data acquisition layer.

Two data sources:
  1. Spotify Web API  — playlist tracks, metadata, added_at timestamps
  2. ReccoBeats API   — audio features (energy, danceability, valence, etc.)
"""
...
RECCOBEATS_BATCH = 10
RECCOBEATS_DELAY = 0.20


class SpotifyClient:
    """Unified client for Spotify and ReccoBeats."""
    ...
    def get_playlist_tracks(self, playlist_url: str) -> list[dict]:
        """Return every track in a playlist with added_at attached."""
        pid = self._parse_playlist_id(playlist_url)
        ...
        while results:
            for item in results.get("items", []):
                ...
                track["added_by_id"] = item.get("added_by", {}).get("id", "")
                tracks.append(track)
    ...
    def get_artist_genres(self, artist_ids: list[str]) -> dict[str, list[str]]:
        """Returns {artist_id: [genre, ...]}. Silently skips on 403."""
        ...
        for i in range(0, len(unique_ids), 50):
            ...
            try:
                for artist in self.sp.artists(batch).get("artists", []):
                    if artist:
                        genres[artist["id"]] = artist.get("genres", [])
            except Exception as e:
                if "403" in str(e):
                    log.warning("Artist/genre endpoint blocked (403) — no genres available.")
                    return {}
                raise
            time.sleep(0.05)
    ...
    def get_audio_features(self, track_ids: list[str]) -> list[dict]:
        """
        Fetch audio features from ReccoBeats via Spotify track IDs.
        Returns one dict per input track_id (empty dict if unavailable).
        """
        ...
        for i in range(0, total, RECCOBEATS_BATCH):
            ...
            try:
                ...
                if resp.status_code == 429:
                    ...
                    log.warning("ReccoBeats rate limit — waiting %ds ...", wait)
                    time.sleep(wait)
                    resp = self.session.get(
                        f"{RECCOBEATS_BASE}/v1/audio-features",
                        params=params, timeout=20,
                ...
                if resp.status_code == 200:
                    ...
                    id_lookup = _build_href_lookup(items)
                    for j, sid in enumerate(batch):
                        feat = id_lookup.get(sid) or (items[j] if j < len(items) else {})
                        all_features.append(_normalise_reccobeats(feat, sid))
                else:
                    for sid in batch:
                        all_features.append(self._reccobeats_single(sid))
                        time.sleep(0.1)

            except requests.exceptions.RequestException as e:
                log.error("ReccoBeats error: %s", e)
            ...
            if done % 200 == 0 or done == total:
                ...
                log.info("  ReccoBeats: %d / %d processed (%d with features)",
                         done, total, valid)

        valid = sum(1 for f in all_features if f.get("energy") is not None)
        log.info("ReccoBeats complete: %d / %d tracks have audio features.", valid, total)
        return all_features

    def _reccobeats_single(self, spotify_id: str) -> dict:
        try:
            resp = self.session.get(
            ...
            if resp.status_code == 200:
                return _normalise_reccobeats(resp.json(), spotify_id)
        except Exception:
            pass
        return {}
...
def _build_href_lookup(items: list) -> dict[str, dict]:
...
def _normalise_reccobeats(feat: dict, spotify_id: str) -> dict:
    if not feat:
        return {}
    return {
        "id":               spotify_id,
