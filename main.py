...
Flags:
    ...
    --contamination F  Outlier threshold, e.g. 0.05 = 5%
    --no-meta          Exclude release_year from UMAP
    --no-browser       Don't open browser automatically
    --export           Save CSV + report to exports/
"""
from __future__ import annotations

# Suppress Intel/LLVM OpenMP conflict and KMeans warnings on Windows
import os
...
from src.processing.stats    import compute_playlist_stats, compute_pca
...
def run(args: argparse.Namespace) -> dict:
    ...
    playlist_id   = SpotifyClient._parse_playlist_id(args.playlist_url)
    log.info("Playlist: '%s'  (id: %s)", playlist_info.get("name", "?"), playlist_id)
    ...
    embedding, _ = compute_umap(
        X,
        n_neighbors = args.n_neighbors,
        min_dist    = args.min_dist,
        metric      = args.metric,
    )
    _, pca_var = compute_pca(X)
    ...
    log.info("Diversity: %.4f | Outliers: %d (%.1f%%)",
             div, int(df["is_outlier"].sum()), 100 * df["is_outlier"].mean())
